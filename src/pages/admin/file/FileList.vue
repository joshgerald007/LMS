    ...
    <template #confirm-delete-modal="a">
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure you want to delete this file?</span>
        </q-card-section>
...
async function getData(filter = {}) {
  ...
  const result = await list(
    `files?search=${filter.search || ''}&per_page=${filter.page?.rowsPerPage || 10}&page=${filter.page?.page || 1}${start}${end}${sBy}${oBy}`,
  )
  loading.value = false
...
async function getExport() {
  const result = await exports(`files`)
  if (result.status === 200) {
    Notify.create({
