<template>
  <div class="q-pb-sm">
    <q-breadcrumbs>
      ...
      <q-breadcrumbs-el label="OR Number" />
    </q-breadcrumbs>
  </div>
  <table-listing
    :columns="columns"
    ...
    <template #confirm-delete-modal="a">
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure you want to delete this OR number?</span>
        </q-card-section>
...
async function deleteItem(id) {
  Loading.show()
  const result = await del('finance/or-number', id)
  Loading.hide()
...
async function getData(filter = {}) {
  ...
  const result = await list(
    `finance/or-number?search=${filter.search || ''}&per_page=${filter.page?.rowsPerPage || 10}&page=${filter.page?.page || 1}${start}${end}${sBy}${oBy}`,
  )
  loading.value = false
...
async function getExport() {
  const result = await exports(`finance/or-number`)
  if (result.status === 200) {
    Notify.create({
