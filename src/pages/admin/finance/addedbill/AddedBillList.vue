<template>
  <div class="q-pb-sm">
    <q-breadcrumbs>
      ...
      <q-breadcrumbs-el label="Added Bill" />
    </q-breadcrumbs>
  </div>
  <table-listing
    :columns="columns"
    ...
    <template #confirm-delete-modal="a">
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure you want to delete this added bill?</span>
        </q-card-section>
...
async function deleteItem(id) {
  Loading.show()
  const result = await del('finance/added-bills', id)
  Loading.hide()
...
async function getData(filter = {}) {
  ...
  const result = await list(
    `finance/added-bills?search=${filter.search || ''}&per_page=${filter.page?.rowsPerPage || 10}&page=${filter.page?.page || 1}${start}${end}${sBy}${oBy}`,
  )
  loading.value = false
...
async function getExport() {
  const result = await exports(`finance/added-bills`)
  if (result.status === 200) {
    Notify.create({
